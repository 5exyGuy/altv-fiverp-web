...
export default class LoginRequestHandler extends RequestHandler {
    public handle(request: NextApiRequest, response: NextApiResponse): void {
        ...
        if (!username || !password || users[username].password !== password) return response.status(401).send('401 Unauthorized');

        const payload = { username };
        const accessToken: string = jwt.sign(payload, 'mysecretaccesstoken', { algorithm: 'HS256', expiresIn: 120 });
        const refreshToken: string = jwt.sign(payload, 'mysecretrefreshtoken', { algorithm: 'HS256', expiresIn: 86400 });
