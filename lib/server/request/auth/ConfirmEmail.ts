...
export default class ConfirmEmailRequestHandler extends RequestHandler {
    public async handle(request: NextApiRequest, response: NextApiResponse): Promise<void> {
        ...
        if (!email || !token)
            return response.status(StatusCodes.BAD_REQUEST).json(JsonMessage.convert('Trūksta duomenų!', MessageType.WARNING));
        ...
        try {
            ...
            response.status(StatusCodes.OK).json(JsonMessage.convert('Elektroninis paštas sėkmingai patvirtintas.', MessageType.SUCCESS));
        } catch (error) {
            response
